...
int main(int argc, char* argv[]) {
  ...
  std::cout << "search sequence of " << max << " ints in vector with "
            << coll.size() << " ints\n";
  ...
  for (int i = 0; i < 5; ++i) {
    ...
    pos = std::search(coll.begin(), coll.end(), sub.begin(), sub.end());
...
#if __has_include(<execution>)
    t0 = std::chrono::steady_clock::now();
    pos = std::search(std::execution::par, coll.begin(), coll.end(),
                      sub.begin(), sub.end());
    t1 = std::chrono::steady_clock::now();
    ...
    t0 = std::chrono::steady_clock::now();
    pos =
        std::search(coll.begin(), coll.end(),
